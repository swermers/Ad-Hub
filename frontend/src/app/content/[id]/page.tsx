...
/** Lightweight error boundary for individual page sections — renders fallback on crash */
class SafeSection extends Component<{ children: ReactNode; label?: string }, { hasError: boolean; msg: string }> {
  constructor(props: { children: ReactNode; label?: string }) {
    super(props);
    this.state = { hasError: false, msg: "" };
  }
  static getDerivedStateFromError(error: Error) { return { hasError: true, msg: error?.message || "Unknown" }; }
  componentDidCatch(error: Error) { console.error(`[SafeSection:${this.props.label}]`, error); }
  render() {
    if (this.state.hasError) {
      return (
        <div className="p-3 bg-[#ffb4ab]/5 border border-[#ffb4ab]/10 rounded-lg text-xs text-[#ffb4ab]/50">
          Section &ldquo;{this.props.label}&rdquo; failed to render: {this.state.msg}
        </div>
      );
    }
    return this.props.children;
...
function ContentDetailInner() {
  ...
  if (piece.generation_metadata) {
    try {
      ...
      // Flatten any nested objects/arrays to strings so they're safe to render
      if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
        for (const [key, val] of Object.entries(parsed)) {
          if (val != null && typeof val === "object") {
            (parsed as Record<string, unknown>)[key] = JSON.stringify(val);
          }
          // Also coerce booleans/numbers to strings for safety
          if (typeof val === "boolean" || typeof val === "number") {
            (parsed as Record<string, unknown>)[key] = String(val);
          }
        }
        metadata = parsed;
      }
  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="flex items-center gap-3 mb-6 text-[10px] text-[#E5E1E4]/30 font-mono flex-wrap">
        <span>BG:{String(bgColor)}</span>
        <span>ACCENT:{String(accentColor)}</span>
        <span>TEXT:{String(textColor)}</span>
      ...
      {/* Generation info */}
      <SafeSection label="generation-info">
      {metadata && (
        <div className="text-xs text-[#E5E1E4]/40 flex items-center gap-3">
          ...
          <span>&middot;</span>
          <span>{String(piece.funnel_stage)} stage</span>
