...
function ProfileManager({
  ...
  const handleImportMarkdown = useCallback(async (file: File) => {
    ...
    setImportError(null);
    try {
      await api.importMarkdownVoiceProfile(file);
      await refreshList();
    } catch {
      setImportError("Could not extract voice profile from file. Try a different file.");
    } finally {
      setImporting(false);
    }
  }, [refreshList]);
          ...
          <div className="px-6 py-4 border-t border-[#353437]/50 space-y-2">
            ...
            <label className="block">
              ...
              <motion.span
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className={`flex items-center justify-center gap-2 w-full py-2.5 rounded-xl text-sm font-medium border border-[#554334] text-[#E5E1E4]/60 hover:border-[#FF9500]/40 hover:text-[#FF9500] transition-colors cursor-pointer ${importing ? "opacity-50 pointer-events-none" : ""}`}
              >
                <span className="material-symbols-outlined text-lg">upload_file</span>
                {importing ? "Analyzing writing style..." : "Import from Markdown"}
              </motion.span>
            </label>
            {importError && (
              <p className="text-xs text-[#ffb4ab] text-center">{importError}</p>
            )}
          </div>
        )}
      </motion.div>
