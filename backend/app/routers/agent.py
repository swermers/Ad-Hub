...
# ─── Bot Content Library (for Claude Computer Use / external agents) ────────
...
def get_content_queue(
    ...
    """Get content ready for posting — formatted for bot consumption.

    Claude Computer Use or any external agent calls this to see what content
    is approved and ready to be posted. Returns platform-specific formatted
    content that can be directly copy/pasted or drafted on social platforms.

    Statuses:
    - approved: Human approved, ready to post
    - ready_to_post: Generated by autonomous loop, ready for bot to draft
    - draft: Generated but not yet approved (for review)
    """
    q = db.query(ContentPiece).filter(ContentPiece.status == status)

    if product_id:
        q = q.filter(ContentPiece.product_id == product_id)
    if platform:
        q = q.filter(ContentPiece.platform == platform)

    pieces = q.order_by(ContentPiece.created_at.desc()).limit(limit).all()

    # Get product names for context
    product_ids = list({p.product_id for p in pieces})
    products = db.query(Product).filter(Product.id.in_(product_ids)).all() if product_ids else []
    product_map = {p.id: p.name for p in products}

    # Check which pieces have platform connections (can be API-posted)
    from app.models.distribution import PlatformConnection
    connections = (
        db.query(PlatformConnection)
        .filter(
            PlatformConnection.product_id.in_(product_ids),
            PlatformConnection.status == "active",
        )
        .all()
    ) if product_ids else []
    # Map: (product_id, platform) → has_connection
    connection_set = {(c.product_id, c.platform) for c in connections}

    results = []
    for piece in pieces:
        has_api = (piece.product_id, piece.platform) in connection_set

        # Build platform-specific formatted content
        formatted = _format_for_platform(piece)

        results.append({
            "id": piece.id,
            "product_id": piece.product_id,
            "product_name": product_map.get(piece.product_id, "Unknown"),
            "content_type": piece.content_type,
            "platform": piece.platform,
            "status": piece.status,
            "has_api_connection": has_api,
            "posting_method": "api" if has_api else "manual",
            ...
            # Platform-ready formatted content (copy/paste ready)
...
@router.post("/content-queue/{content_id}/mark-drafted")
def mark_content_drafted(
    content_id: str,
    platform_draft_url: str | None = None,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_current_user),
):
    """Mark a content piece as drafted on a social platform.

    Called by Claude Computer Use after it saves content as a draft
    on a social platform. Updates status and optionally stores the
    platform draft URL for the human to review and publish.
    """
    piece = db.query(ContentPiece).filter(ContentPiece.id == content_id).first()
    if not piece:
        raise HTTPException(404, "Content piece not found")

    piece.status = "drafted_on_platform"

    # Store draft URL in metadata
    meta = {}
    if piece.generation_metadata:
        try:
            ...
            pass
    meta["platform_draft_url"] = platform_draft_url
    meta["drafted_at"] = datetime.now(timezone.utc).isoformat()
    meta["drafted_by"] = "bot"
    ...
    _notify_draft_ready(piece, platform_draft_url)

    return {
        "id": content_id,
        "status": "drafted_on_platform",
        "platform_draft_url": platform_draft_url,
        "message": "Content marked as drafted. User notified.",
    }


@router.post("/content-queue/{content_id}/mark-posted")
def mark_content_posted(
    ...
    """Mark content as posted (by bot or manually)."""
    piece = db.query(ContentPiece).filter(ContentPiece.id == content_id).first()
    if not piece:
        raise HTTPException(404, "Content piece not found")
    ...
    if piece.generation_metadata:
        try:
            ...
            pass
    meta["platform_post_url"] = platform_post_url
    meta["posted_at"] = datetime.now(timezone.utc).isoformat()
    piece.generation_metadata = json.dumps(meta)

    db.commit()

    return {"id": content_id, "status": "posted"}


@router.get("/content-queue/summary")
def get_queue_summary(
    product_id: str | None = None,
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_current_user),
):
    """Quick summary of content in each status — useful for bot decision-making."""
    from sqlalchemy import func

    q = db.query(ContentPiece.status, ContentPiece.platform, func.count(ContentPiece.id))

    if product_id:
        q = q.filter(ContentPiece.product_id == product_id)

    counts = q.group_by(ContentPiece.status, ContentPiece.platform).all()

    summary = {}
    for status, platform, count in counts:
        if status not in summary:
            summary[status] = {"total": 0, "by_platform": {}}
        summary[status]["total"] += count
        summary[status]["by_platform"][platform] = count

    return {"summary": summary}


def _format_for_platform(piece: ContentPiece) -> str:
    """Format content for direct copy/paste to a specific platform."""
    platform = piece.platform or "general"
    body = piece.body or ""
    hook = piece.hook or ""
    cta = piece.cta or ""

    if platform == "twitter":
        # Twitter: hook + body, trimmed to 280 chars
        ...
        # LinkedIn: hook as opener, body, CTA at end
        parts = []
        if hook:
            parts.append(hook)
        parts.append(body)
        ...
        # Meta/Facebook: conversational, hook + body + CTA
        parts = []
        if hook:
            parts.append(hook)
        parts.append(body)
...
def _notify_draft_ready(piece: ContentPiece, draft_url: str | None):
    """Send Telegram notification that a draft is ready for review."""
    import asyncio
    import threading
    ...
    def _send():
        try:
            ...
            title = piece.title or piece.body[:50]
            msg = (
                f"<b>New draft ready on {platform.upper()}</b>\n\n"
                f"<i>{title}</i>\n"
            )
            if draft_url:
                msg += f"\nOpen draft: {draft_url}"
